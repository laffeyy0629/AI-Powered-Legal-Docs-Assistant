...
export default function ForgotUsername() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post('/user/forgot-username', { email: email.trim() });

      if (response.success) {
        setSuccessMessage(response.message || 'Username reminder sent! Please check your email.');
        setEmail(''); // Clear form
      } else {
        setError(response.message || 'Failed to send username reminder.');
      }
    } catch (err) {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 via-gray-800 to-black px-4 overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute -bottom-40 -left-40 w-80 h-80 bg-pink-500/5 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }}></div>
      </div>
        ...
        <div className="bg-gray-800/50 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-700/50 p-8">
            ...
            >
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-300 mt-1">Check your inbox and spam folder.</p>
                </div>
              </div>
            </motion.div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-semibold py-3 px-4 rounded-lg transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
            >
              {loading ? (
